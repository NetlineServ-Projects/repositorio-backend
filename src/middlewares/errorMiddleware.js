...
function errorMiddleware(err, req, res, next) {
    console.error(err); // log interno, útil em desenvolvimento

    // Erro conhecido (lançado intencionalmente por um service)
    if (err.statusCode) {
        return response.error(res, err.message, err.statusCode);
    }

    // Erro do Prisma - registo duplicado (ex: email já existe)
    if (err.code === "P2002") {
